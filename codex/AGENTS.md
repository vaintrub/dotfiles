...
Never write to `~/.claude/<X>` or `~/.codex/<X>` directly — those are symlinks into the dotfiles repo. The full symlink table and per-area guidance live in `~/dotfiles/AGENTS.md`, which Codex auto-loads when cwd is in that repo.
